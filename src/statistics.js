...
const REWARDS = [
  {
    ...
    emoji: "⏰",
    cost: 100,
    desc: "Shows superior punctuality.",
  },
  {
    ...
    emoji: "🌟",
    cost: 100,
    desc: "Awarded for persistent effort.",
  },
  {
    id: "badge_focus",
    name: "Laser Focus",
    emoji: "🎯",
    cost: 100,
    desc: "Represents sharp task execution.",
  },
  {
    id: "badge_velocity",
    name: "Speed Runner",
    emoji: "⚡",
    cost: 100,
    desc: "You move fast and finish early.",
  },
  {
    id: "badge_resilience",
    name: "Unstoppable",
  ...
  },
  {
    ...
    emoji: "📅",
    cost: 100,
    desc: "Meticulous scheduling skills.",
  },
];
...
onAuthStateChanged(auth, (user) => {
  ...
  Promise.all([loadStats(currentUser.uid), loadPoints(currentUser.uid)]).catch(
  ...
  REWARDS.forEach((r) => {
    ...
    card.innerHTML = `
      <div class="border rounded p-3 h-100 d-flex flex-column" style="background:#fff; box-shadow:0 2px 8px rgba(0,0,0,0.08);">
        ...
        <div class="mb-2"><span class="badge bg-warning text-dark">Cost: ${
          r.cost
        }</span></div>
        <div class="mt-auto d-flex flex-wrap gap-2">
          ${
            owned
              ? `<button class="btn btn-sm btn-outline-secondary equip-btn" data-id="${
                  r.id
                }" ${equipped ? "disabled" : ""}>${
                  equipped ? "Equipped" : "Equip"
                }</button>`
              : `<button class="btn btn-sm btn-primary buy-btn" data-id="${
                  r.id
                }" ${canAfford ? "" : "disabled"}>${
                  canAfford ? "Buy" : "Need " + (r.cost - rewardState.points)
                }</button>`
          }
        </div>
      </div>`;
...
async function equipReward(rewardId) {
  ...
  try {
    await updateDoc(doc(db, "userRewards", currentUser.uid), {
      equippedBadge: rewardId,
    });
...
async function loadStats(uid) {
  ...
  snap.forEach((doc) => {
    const d = doc.data();
    const ts =
      d.timestamp && d.timestamp.toDate ? d.timestamp.toDate() : new Date();
    let plotDate = ts;
    try {
      const meta = d.meta || {};
  ...
  els.kpiOnTime.textContent = arrivalTotal
    ? Math.round((arrivalOnTime / arrivalTotal) * 100) + "%"
    : "—";
  els.kpiOnTimeMeta.textContent = arrivalTotal
    ? `${arrivalOnTime} / ${arrivalTotal} arrivals on time`
    : "No arrival data";
    ...
    item.innerHTML = `
      ...
      <div class="timeline-content">
        ...
        <div class="timeline-meta">
          ...
          ${
            meta.scheduledTime
              ? `<span><i class="fas fa-calendar"></i> Scheduled: ${new Date(
                  meta.scheduledTime
                ).toLocaleString()}</span>`
              : ""
          }
